...
def extract_coordinates_nms(heatmaps, downsample_factor=8, conf_threshold=0.1, kernel_size=3):
    ...
    for b in range(heatmaps.shape[0]):
        ...
        # FIX: Changed "confidence" to "conf" to prevent KeyError
...
def stitch_to_whole_slide(slice_predictions, distance_threshold=10.0):
    """Merges overlapping predictions using Global NMS and preserves confidence."""
    ...
    for i in range(len(global_points)):
        ...
        # FIX: Save both the coordinate and the confidence score
        final_results.append({
            'coord': global_points[i].tolist(),
            'conf': float(global_scores[i])
        })
...
def run_inference_pipeline(image_path, model_weights_path, output_path, patches_dir=None, device=None):
    if device is None:
    ...
    with torch.no_grad(): # No gradients needed for inference!
        ...
        if batch_tensors:
            ...
            pred_heatmaps, _ = model(inputs)
            pred_heatmaps_sig = torch.sigmoid(pred_heatmaps)
            batch_results = extract_coordinates_nms(pred_heatmaps_sig, downsample_factor=8, conf_threshold=0.30)
            for i, result in enumerate(batch_results):
                pts = [[p['x'], p['y']] for p in result]
                scores = [p['conf'] for p in result]
                slice_predictions.append({
    ...
    # 6. Visualization
    """print("Drawing prediction dots...")
    
    # Dynamic scaling just for the dot
    scale = max(1, img_w // 800)
    center_dot = max(3, 3 * scale)

    for idx, item in enumerate(final_global_coordinates):
        # Unpack the dictionary
        pt = item['coord']
        gx, gy = int(pt[0]), int(pt[1])
        
        # Draw ONLY the bright green center dot (-1 means it is filled in completely)
        cv2.circle(draw_image, (gx, gy), center_dot, (0, 255, 0), -1)"""
    
    # 6. Visualization
    print("Drawing prediction targets...")
    
    # Dynamic scaling for BOTH the circle and the dot
    scale = max(1, img_w // 800)
    radius = 15 * scale      # Circle size
    thickness = 3 * scale    # Line thickness
    center_dot = max(3, 3 * scale) # Dot size

    for idx, item in enumerate(final_global_coordinates):
        pt = item['coord']
        gx, gy = int(pt[0]), int(pt[1])
        
        # 1. Draw the Cyan Circle first
        cv2.circle(draw_image, (gx, gy), radius, (255, 255, 0), thickness) 
        
        # 2. Draw the solid Green Center Dot on top (-1 makes it filled)
        cv2.circle(draw_image, (gx, gy), center_dot, (0, 255, 0), -1)

    # ... (Keep the patches_dir saving code below this) ...
    ...
    # FIX: Return the full dictionary structure so the GUI gets the confidence scores
    return final_global_coordinates
...
if __name__ == "__main__":
    ...
    MODEL_WEIGHTS = "./saved_models/hpylori_best_model (2).safetensors"
